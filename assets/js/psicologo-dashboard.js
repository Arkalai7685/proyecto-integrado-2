document.addEventListener('DOMContentLoaded', () => {
    ...
    // initializePsychologistCalendar(); // Comentado temporalmente
    initializeFolders();
...
function initializePsicologoCalendar() {
    ...
    const appointments = {
        '2025-10-23': [
            { time: '10:00', student: 'María García', type: 'terapia' },
            { time: '14:30', student: 'Carlos López', type: 'consulta' }
        ],
        '2025-10-25': [
            { time: '11:00', student: 'Ana Martínez', type: 'terapia' }
        ],
        '2025-10-28': [
            { time: '09:00', student: 'Pedro Rodríguez', type: 'seguimiento' },
            { time: '15:00', student: 'Laura Silva', type: 'evaluación' }
        ],
        '2025-11-02': [
            { time: '10:30', student: 'Miguel Torres', type: 'terapia' }
        ],
        '2025-11-05': [
            { time: '13:00', student: 'Sofia Mendez', type: 'consulta' }
        ]
    };
...
function initializeStudentCards() {
    ...
    // que llama a la función definida en psicologo-dashboard.html
}
...
function verArchivosCliente(clientId) {
    console.log('=== verArchivosCliente llamada ===');
...
function loadClientFilesPsico(clientId) {
    ...
    console.log('Iniciando fetch a:', `/api/file/list/?client_id=${clientId}`);
    ...
    .then(response => {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            throw new Error('La respuesta no es JSON. El servidor devolvió HTML (posible error 404/500)');
        }
    ...
    .catch(error => {
        ...
        errorDiv.innerHTML = '<div style="font-size: 56px; margin-bottom: 15px;">❌</div><h3 style="color: #e74c3c; margin-bottom: 10px;">Error al cargar archivos</h3><p style="color: #999;">Por favor, intenta de nuevo más tarde</p><p style="color: #999; font-size: 12px; margin-top: 10px;">' + error.message + '</p>';
